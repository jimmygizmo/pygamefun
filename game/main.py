...
SurfCacheItem = TypedDict('SurfCacheItem',
    {
        ...
        'mask_surf_l': pygame.Surface,  # Mask Surface LEFT  -  # Used by whiteout effect and later, possibly other FX.
        'mask_surf_r': pygame.Surface,  # Mask Surface RIGHT  -  # "
    }
)  # SurfCacheitem
SCACHE: dict[str, SurfCacheItem] = {}  # The Surface Cache. Key = filename, Value = SurfCacheItem.
# end SURFACE CACHE global initialization  -  #
#   The Surface Cache SCACHE pre-loads images into surfaces. When sprites are instantiated, they will use this cache and
#   not need to load from disk all the time, greatly impacting performance. This is important for
#   dynamically/frequently spawned/destroyed sprites.
#
#   TODO: Document how filename and frames_dir are unique and thus are used as primary keys for image/frame data.
#     (both are relative paths under /assets/img, /assets/anim and are files/directories .. hence unique there.)
...
# end AUDIO CACHE global initialization  -  # TODO: This is a brand new feature. Same concept: avoid slow storage I/O.
...
class Entity(pygame.sprite.Sprite):
    ...
    def update(self, delta_time: float, ephase_name: str | None):
        ...
        #   use pre-computed/stored images. Raw pixels are manipulated so this is an area of performance concern,
        #   especially since this occurs every frame for some Entity instances.
        #   TODO: delta_time is too small to directly multiply against rotation. But I do this for linear speeds:
        #     I think those linear speeds are artificially large to account for multiplying directly against this
        #     delta_time (about 0.016). But for a rotation, we cannot change 0-360 so we must address this delta_time
        #     scaling issue.
...
new_greenballs_groups: list[pygame.sprite.Group] = [all_greenballs]
new_meatballs_groups: list[pygame.sprite.Group] = [all_meatballs]
greenball_anim_groups: list[pygame.sprite.Group] = [all_anims]
meatball_anim_groups: list[pygame.sprite.Group] = [all_anims]
# NOTE: We can assign Sprites to multiple groups, but currently we only assign to one. This could change. We don't
#   have a need for an all_sprites since we draw one type at a time to get the desired layering. Also, an all_colliders
#   group was considered to manage all collisions from one point, but this was decided against. So the above group
#   lists used to have 3 groups in each (like: all_sprites, all_anims, all_colliders) but now have only one each.
...
for i, gr_player_spec in enumerate(ent.player_specs):
    ...
    # TODO: UPDATE THESE COMMENTS AND MOVE THE REMAINDER BELOW. THIS IS OUT-DATED NOW. ANIMATION IS FARILY STABLE/DEPLOYED.
