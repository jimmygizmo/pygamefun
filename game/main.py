...
SurfCacheItem = TypedDict('SurfCacheItem',
    {
        ...
        'mask_l': pygame.Mask,  # Mask from surface LEFT - TODO: Consider not storing in cache
        'mask_r': pygame.Mask,  # Mask from surface RIGHT - TODO: Consider not storing in cache
        'mask_surf_l': pygame.Surface,  # Mask Surface LEFT
...
class Entity(pygame.sprite.Sprite):
    ...
    def __init__(self,
        ...
        #
        # TODO: mask_l and mask_r should now be available in the SCACHE. Need to test before switching over.
        # self.mask_l: pygame.Mask = pygame.mask.from_surface(self.surface_l)  # Mask from surface LEFT
        # self.mask_r: pygame.Mask = pygame.mask.from_surface(self.surface_r)  # Mask from surface RIGHT
        ...
        # TODO: Add these to SCACHE:
        # self.mask_surf_l: pygame.Surface = self.mask_l.to_surface()  # Now a 'mask surface' from that (black & white silhouette)
        # self.mask_surf_r: pygame.Surface = self.mask_r.to_surface()  # Now a 'mask surface' from that (black & white silhouette)
        self.mask_surf_l: pygame.Surface = SCACHE[img_filename]['mask_surf_l']  # Now a 'mask surface' from that (black & white silhouette)
        self.mask_surf_r: pygame.Surface = SCACHE[img_filename]['mask_surf_r']  # Now a 'mask surface' from that (black & white silhouette)
        # TODO: ABOVE HERE - These 4 items will soon come from SCACHE !!!
        #
        self.x: float = x
        self.y: float = y
        ...
        if cfg.WHITEOUT_MODE:
            # Early Mask Implementation, examples:    # TODO: (probably) Add Mask support into the GLOBAL SURFACE CACHE: SCACHE
            # TODO: A common left/right mask would work for many objects and is a feature worth supporting. Call it 'simple masking'.
            self.mask_surf_l.set_colorkey((0, 0, 0))  # Then make the black transparent, just for fun.
            self.mask_surf_r.set_colorkey((0, 0, 0))  # Then make the black transparent, just for fun.
            self.surface_l = self.mask_surf_l  # JUST FOR FUN - WHITE-OUT THE OBJECT
            self.surface_r = self.mask_surf_r  # JUST FOR FUN - WHITE-OUT THE OBJECT
...
def load_image(
    ...
    # TODO: We might only need to store the mask surface in SCACHE and not the mask,
    #      but I need think this over as it is a new idea.
